...
> **Housekeeping, 2026-08-22 (third pass).** Several sessions ran concurrently against this same branch and each independently hit and "fixed" the Phase 22 collision below, in opposite directions, across two separate merges — one pass moved the build-pipeline phase to 23, a later pass moved it back and pushed Hot, Honest, Ours to 23 instead, and a `git merge` of non-overlapping line insertions combined both fixes rather than flagging either as a conflict, leaving two `## Phase 23` headers and two duplicated `**Sources:**` lines at once. Settled here, finally: **build-pipeline integrity stays Phase 22** (its original number), **Hot, Honest, Ours stays Phase 23**. `FLAG-15`, the one row that had been carried in both duplicate sections, is now resolved (branch protection enabled, 2026-08-22) and moved to `completed.tasks.md` — Phase 22's table below is empty on purpose, matching the bare-pointer convention every other fully-shipped phase already uses (13–19, 21).
>
...
| ~~**WD-29**~~ | ~~The Consent Domains Map's five-value answer scale codes `no` as rust — the "where the framework fails" register — against thesis 09, "Refusal is a relational act."~~ | `[DESIGN]` `[DECISION]` | — | 🟢 **Resolved 2026-08-22 — kept as-is.** Revisited once Hot, Honest, Ours (Phase 23) gave the same question a second, sharper instance (its Boundary tier uses the identical rust register). The author's call covers both: read rust as this site's established "safety surface" register (the Practise rust carve-out, below), not literally "the framework failed." No colour change, either place. See `tasks.md`'s HHO-19 for the newer instance. |
...
## Phase 23 — Hot, Honest, Ours: local-first redesign (self-directed, 2026-08-22)
...
| **HHO-11** | Route wiring, complete: `scripts/check-origins.mjs` (shipped earlier), a `responsive-audit.mjs` screenshot entry, and a standalone `checkHotHonestOurs()` in `scripts/check-pages.mjs` (HTML validity, axe at desktop and phone width, reflow/target-size, CSP-violation listener) — kept separate from the shared `pages`/`GROUND` loop rather than joining it, since this page shares neither `head-base.html`'s ground colour nor the rest of the site's CSP, and silently joining `GROUND` would have skipped its own reflow pass rather than exempted it on purpose. `scripts/prerender.mjs` and `scripts/sync-base.mjs` confirmed **not applicable** (spec §9.2). | `[DEV]` | `scripts/check-pages.mjs`, `responsive-audit.mjs` | — | Shipped — see `completed.tasks.md` for a real, previously-invisible bug this check found on its first run. |

### Mechanism — remaining

| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **HHO-09 (residual)** | Comparison results UI shipped with cards + triage tiles + boundary-first ordering + the closing line, per spec §8.1's keep/change/reject list applied to the supplied `Compare_Sheet.dc.html`. **Grid layout for dense same-scale groups now shipped** (spec §8.4b) — the Want Menu's 13 closeness marks and 10 activity-scale rows, Bandwidth Check's 8 capacity marks, and the Buffet's 13 rows render as real `<table>`s (row headers, explicit ARIA roles so a CSS display override on narrow screens can't strip table semantics) grouped by tier, instead of one card each. Verified live: 13 tables rendered across a real two-file comparison with matched/differ/boundary/solo rows correctly distributed and merged only within a shared tier bucket, 0 axe violations at 1000px and 375px, no horizontal overflow, thead visually-hidden (not removed) below 700px. **Still not built:** ordinal dot-plots with text equivalents and demo mode — both remain "Should Have — cards work meanwhile" in the spec's own §14 roadmap. | `[DEV]` | `practise/hot-honest-ours/{app.js,style.css,questions.js,engine.js}` | S | |

### Content — the twelve rounds

| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **HHO-12** | The door (cover, safety five, access check) and all twelve rounds' real content — 121 questions plus 8 access-check rows, transcribed from the source worksheet, replacing the nine representative ones. Shipped — see `completed.tasks.md` for the exact simplifications made against the fuller UX spec (Round 5's "changes often" folded into the mark scale, Round 6's aftercare as a chip-plus-note rather than a card triptych, Round 7's untangle not repeatable, Round 10's per-yes fridge-five reveal not built, Round 2's "category only" flag not built) and the type-system additions (`help`, `note`→`c`, `placeholder`, `neverShareable`, the `choice` type). | `[DEV]` `[COPY]` | `practise/hot-honest-ours/*` | — | Round 9 ("Three Words") is reference-only, matching the source worksheet exactly — it has no blanks to fill in the original either. |
| **HHO-13** | Carry-over group in Round 8, shipped — see `completed.tasks.md`. All five Consent Domains Map domains the worksheet doesn't otherwise cover (location sharing, passwords & devices, shared property, cultural knowledge, administrative/logistical care), using the map's own names, hints, and five-option scale, per UX spec §13.2. | `[DEV]` `[COPY]` | same | — | Cultural knowledge is included — RS-028's anti-appropriation guardrail rests on it being asked. |
| **HHO-14 (residual)** | **The "choose your way in" screen and the fridge five are shipped** — see `completed.tasks.md`. The door now leads to a real menu (spec §4.1/§4.2) rather than straight into the twelve rounds; "the long way round" and "the fridge five" are both real choices from it. **The 60-second check-in and the shuffle are not built** — see `HHO-27`, a real content gap found while scoping this row, same shape as `HHO-26`. | `[DEV]` `[COPY]` | `practise/hot-honest-ours/*` | S | The menu currently offers two of the spec's four modes; the other two are named in its own fine print rather than hidden. |
| **HHO-27** | **A content gap found while scoping HHO-14, before building anything.** The 60-second check-in's eight questions (spec §6.16: 2 scale rows, 4 writing fields, 2 yes/no-ish rows) and the shuffle's twelve prompts (spec §6.18, grouped into three named games — Green Flag Round, Two Truths and a Limit, The Re-Order) are described only by shape in `docs/spec/hot-honest-ours.md`; their actual wording is never quoted. Unlike the Fridge Five (whose five category labels and closing reading are quoted verbatim in the spec's own §1.3/§3.3/§6.17) or Round 9 (`HHO-26`, resolved the same way), there is no verbatim source material anywhere in this repo to build from — the original prototype (`Hot_Honest_and_Ours.dc.html`) was only ever pasted into an earlier session, never saved. | `[DECISION]` `[COPY]` | `practise/hot-honest-ours/{questions.js,index.html,app.js}` | M | Needs either the original prototype content, or an explicit call to draft new, clearly house-authored content matching the spec's shape (a bigger departure from this project's "verbatim from source" convention than `HHO-26`'s narrower glossary reconstruction was). The verdict row's own five words ("good to go · slow down · pause · renegotiate · stop") and the shuffle's three game names are already verbatim and don't need this. |

### Site integration and cleanup

| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **HHO-15** | **Shipped, side by side rather than a replace** (author call, 2026-08-22 — see `completed.tasks.md`). `/practise/index.html` gained a new tool-03 card for Hot, Honest, Ours (copy per `docs/spec/hot-honest-ours.md` §8.1, adapted since the map stays), a `#hot-honest-ours` nav link, and the page's tool-count claims corrected from nine to ten (this is a genuinely new tool, not one of the seven already-named unwritten ones). `#consent-map` is untouched. | `[DEV]` `[COPY]` | `practise/index.html` | — | |
| **HHO-16** | Moving/retiring the Consent Domains Map — **deferred, not shipped.** The author chose to ship side by side (HHO-15) rather than replace, specifically because HHO-14 (standalone modes) isn't built yet. Revisit once HHO-14 ships and whether to retire the map becomes a live question again. | `[DEV]` | new path, `practise/index.html` | S | Not the same as rejected — see `docs/spec/hot-honest-ours-privacy-architecture.md`'s own convention for naming a deferral. |
| **HHO-17** | `index.html` roadmap line under "Open now" — shipped. Added "Hot, Honest, and Ours" alongside "The Consent Domains Map" rather than replacing it, matching HHO-15's side-by-side call. | `[DEV]` `[COPY]` | `index.html` | — | |
| **HHO-18** | **IA-11 update — shipped.** Learn's "Take it further" link ("Work through the Consent Domains Map to apply them") now points at Hot, Honest, and Ours instead — the broader, still-live tool is the better single "apply this" recommendation, independent of whether the map itself is ever retired. | `[DEV]` | `hugo/layouts/learn.html`, `learn/index.html` | — | **Caveat:** `hugo` isn't installed in this sandbox, so the edit was applied identically by hand to both the layout source and the shipped output (a literal string, no template logic around it) rather than verified via a real `npm run build:hugo`. Re-run that build once available to confirm byte-for-byte sync. |
| **HHO-19** | **WD-29 revisit — resolved, 2026-08-22.** Author call: keep rust for the Boundary tier, read as this site's established "safety surface" register (Practise's own rust carve-out, `archived.tasks.md`) rather than literally "the framework failed." WD-29 itself closed on the same call — see its own row in Phase 9 above. | `[DECISION]` | — | — | Closed; full reasoning in `archived.tasks.md`. |
| **HHO-20** | **HUGO2-03 scope note**, corrected for the side-by-side call: that task's file list ("the domain map, endings tool") needs to account for *three* built tools now, not a map-for-HHO swap — don't let the Hugo migration template a page that no longer exists, and don't assume the map is gone. | `[DECISION]` | `hugo/layouts/practise.html` (when HUGO2-03 is picked up) | — | Note only; HUGO2-03 itself is unstarted. |
| **HHO-21** | **Resolved, 2026-08-22 — no change needed, and now permanent rather than conditional.** Author confirmed V1 stays tier 0 (memory only, HHO-24) — `/practise/`'s existing "nothing is stored" privacy paragraph stays accurate as written. | `[DECISION]` | — | — | Closed. |

### Open decisions (defaults recorded, not yet confirmed by the author)

| ID | Question | Default already recommended | Status |
|---|---|---|---|
| **HHO-22** | Path under `/practise/` vs. a separate `ours.` subdomain. | Path, for V1 — spec §9.1. Revisit if HHO-24 ships tier 2. | 🟢 **Resolved, 2026-08-22 — path confirmed.** Revisit only if HHO-24 is ever reopened past tier 0. |
...
| **HHO-24** | Storage tiers beyond memory-only (`sessionStorage`, encrypted IndexedDB). | Tier 0 (memory only) for V1 — keeps `/practise/`'s existing privacy sentence true with no copy change. | 🟢 **Resolved, 2026-08-22 — memory-only confirmed.** The reload-loses-your-draft cost was weighed and accepted rather than traded away. |
...
| **HHO-26** | **Resolved and shipped, 2026-08-22 — see `completed.tasks.md`.** Round 9 ("Three Words People Use Interchangeably and Shouldn't") now exists: reference only, no input, built from the spec's own §1.3 terminology table (Boundary → what I do, Agreement → what we chose, Rule → an attempt to control) rather than the original worksheet's exact wording, which was never saved anywhere in this repo. Author call: the glossary version is good enough; not held open for the exact original text. | `[DEV]` `[COPY]` | `practise/hot-honest-ours/{questions.js,app.js,style.css}` | — | If the exact source text ever surfaces, replace `r9.words`'s `terms` in `questions.js` with it verbatim — flagged inline in the code. |
...
- `docs/spec/hot-honest-ours.md` — Phase 23's UX/interaction spec: the room, the door, all twelve rounds, screen-by-screen, verbatim source copy throughout
- `docs/spec/hot-honest-ours-privacy-architecture.md` — Phase 23's sharing/comparison/crypto spec; supersedes the UX spec at its own §7.9 and §9.3–9.5; its own §0.1 (2026-08-22) records a right-sizing pass that cut cryptography the site's actual threat model didn't call for — read it before touching any HHO- row
