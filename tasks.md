...
|---|---|---|---|---|---|
| **RS-004** | Migrate `Learn`, `Archive`, `BehindTheScenes` (and any other reading page) to Hugo-generated static HTML, committed to the repo as the shipped artifact. `Practise.dc.html` keeps the runtime — it's inherently interactive. **Manifesto done, see `completed.tasks.md`** — toolchain, `npm run build:hugo`, and the CI sync-check all exist now, so each remaining page is "write the data file + layout, add to `HUGO_PAGES`," not new infrastructure. Retire fault 04 (colophon) once all three remaining pages land, not before — it's a claim about the reading pages collectively. | `[DEV]` | `hugo/`, `Learn.dc.html`, `Archive.dc.html`, `BehindTheScenes.dc.html` | L (was L; toolchain now sunk cost, remaining is 3 real data-driven migrations, Archive being the largest) | ~~`[VERIFY]` Hugo current version/platform availability (D2)~~ — done, v0.164.0, see `completed.tasks.md` |
