...
**Sequencing note (mine, not in the source docs):** RS-004 is a full migration off the current `dc-runtime`/`support.js` template system onto a Hugo-generated static build. Every other Phase-1 item edits page content. If content edits land in the current JS-array format *before* RS-004 migrates that page, the same content gets authored twice — once now, once during migration. My recommendation: **build RS-001+032 and RS-002+031 first** (they're new interactive surfaces, not migrations of existing arrays, so there's nothing to duplicate — **done, see `completed.tasks.md`**), **then RS-004**, **then RS-028+RS-005** as new Hugo-native content once the target format exists. **RS-030 shipped ahead of that sequence** (see `completed.tasks.md`) — it's one self-contained clause, not a migration of an existing array, so the duplication risk the sequencing note is about doesn't apply the way it does to RS-028+005 (which touches the Practise tool's domain array *and* the principles array across two pages). RS-020 and RS-023 run alongside throughout since they're cross-cutting.

Note: main separately renamed `Colophon.dc.html` to `BehindTheScenes.dc.html` (merged into this branch 2026-08-07) — RS-004's target file list below is updated for that.

| ID | Task | Tags | Files | Effort | Depends on |
|---|---|---|---|---|---|
| **RS-004** | Migrate `Archive`, `BehindTheScenes` (and any other reading page) to Hugo-generated static HTML, committed to the repo as the shipped artifact. `Practise.dc.html` keeps the runtime — it's inherently interactive. **Manifesto and Learn done, see `completed.tasks.md`** — toolchain, `npm run build:hugo`, and the CI sync-check all exist now, so each remaining page is "write the data file(s) + layout, add to `HUGO_PAGES`," not new infrastructure. Retire fault 04 (colophon) once both remaining pages land, not before — it's a claim about the reading pages collectively. | `[DEV]` | `hugo/`, `Archive.dc.html`, `BehindTheScenes.dc.html` | L (toolchain now sunk cost; Archive is the largest remaining piece — ~30 entries, groups, the dual-axis diagram from fault 05) | ~~`[VERIFY]` Hugo current version/platform availability (D2)~~ — done, v0.164.0, see `completed.tasks.md` |
