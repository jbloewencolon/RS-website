...
|---|---|---|---|---|---|
| **RS-004** | Migrate `Archive.dc.html` (last reading page) to Hugo-generated static HTML, committed to the repo as the shipped artifact. `Practise.dc.html` keeps the runtime — it's inherently interactive. **Manifesto, Learn, and BehindTheScenes done, see `completed.tasks.md`.** Retire fault 04 (colophon) once Archive lands — per D2's own instruction, only once all reading pages are migrated, not before. | `[DEV]` | `hugo/`, `Archive.dc.html`, `BehindTheScenes.dc.html` (fault 4 retirement + changelog entry) | M (toolchain and pattern both proven three times now; Archive's size — ~30 entries, groups, the dual-axis diagram from fault 05 — is the only real unknown left) | ~~`[VERIFY]` Hugo current version/platform availability (D2)~~ — done, v0.164.0, see `completed.tasks.md` |
