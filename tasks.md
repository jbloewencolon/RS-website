...
| ID | Task | Tags | Files | Effort | Depends on |
|---|---|---|---|---|---|
| **SEC-01.1** | Per-IP **and** per-address rate limits on `/api/subscribe`. Two limits, because they stop different attacks: per-IP (~5/hr) stops one machine; per-address (one pending confirm per address per hour) stops a distributed flood burying one victim, which per-IP will not catch. KV is sufficient; a Durable Object is more precise. | `[DEV]` | `worker/src/index.js`, `wrangler.toml` | M | — |
| **SEC-01.2** | Global daily send ceiling as a circuit breaker. Caps the Resend bill and the reputation damage; converts an unbounded incident into a bounded one. Fail closed and alert rather than silently absorbing. | `[DEV]` | `worker/src/index.js` | S | SEC-01.1 |
| **SEC-01.3** | Cloudflare Turnstile on both forms. The honeypot stops bots that fill every field; it stops nothing that reads the form once. Turnstile sets no cookies and does no behavioural profiling — which is why it fits here and reCAPTCHA does not. Keep the honeypot alongside it. | `[DEV]` | `index.html`, `contribute/index.html`, `worker/src/index.js` | M | — |
...
- **Live rate-limit behaviour under load** — not probed. Sending real traffic at a production endpoint that sends real email to real addresses is not a safe thing to do from here; the loop in SEC-01's verification block is for the operator to run against addresses they control.
