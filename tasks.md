...
1. **WD-15, remaining half** — **Practise** needs a built/unbuilt call per tool before its registers can be assigned (the spec is explicit this must not be inferred from the data file), and the rust carve-out in §9.4 applies. **Behind the Scenes** is a genuine open question rather than pending work: all 5 faults in `faults.yaml` are currently unfixed, so colour-coding them would render them uniformly and say nothing. It needs either a per-fault status field or a decision that the distinction isn't worth encoding here.
2. **WD-18** — the `--sans` stack, still gated on RS-024's cross-platform glyph check (FLAG-08). Genuinely cannot be verified in this environment.
...
| **WD-15** | Palette rollout. **Resources and Archive shipped 2026-08-11 (see WD-16); Behind the Scenes and Practise still open.** | `[DECISION]` | **Resources** — the one genuinely empty category (`mutual-aid`) now carries the ochre register on its dashed rule and `Not yet built` scope label. No author judgement was needed: `resources.yaml` already states `scope: "Not yet built"` and the category already had an explanatory paragraph, so this only added the colour the rest of the site uses for that same claim. **Behind the Scenes** is deliberately not done — all 5 remaining faults in `faults.yaml` are currently unfixed, so a green/ochre/rust split would render them all one colour and say nothing; it needs either a real status field per fault or a decision that the distinction isn't worth encoding. **Practise** still needs the author's built/unbuilt call per tool, and the rust carve-out below still applies. |
...
### Phase 10.1 — Close the open endpoint `week 1` — ✅ code done 2026-08-11, blocked on operator setup for go-live

**Sequenced before the header work deliberately.** Every other finding harms the operator or the site's own readers. This one lets a stranger use the site as a weapon against a third party who has never visited it — subscription bombing recruits exactly this shape of endpoint. For a project whose entire argument is about not doing things to people without their consent, this is the finding that matters most, and it is the one the external audit does not mention.

**FLAG-08 companion decision, resolved 2026-08-11:** shipping SEC-01.3 makes the site's first third-party request (Cloudflare's Turnstile script). Asked the author directly rather than assuming; answer was **add Turnstile now, and update the "zero third-party requests" claim rather than avoid the tradeoff.** Reflected in `index.html`/`contribute/index.html` footers and `hugo/data/substrate.yaml`'s "Third-party requests" row, not just here.

| ID | Task | Tags | Files | Effort | Depends on |
|---|---|---|---|---|---|
| **SEC-01.1** | Per-IP **and** per-address rate limits on `/api/subscribe`. Two limits, because they stop different attacks: per-IP (~5/hr) stops one machine; per-address (one pending confirm per address per hour) stops a distributed flood burying one victim, which per-IP will not catch. KV is sufficient; a Durable Object is more precise. | `[DEV]` ✅ | `worker/src/ratelimit.js` (new), `worker/src/index.js`, `worker/wrangler.toml` | M | — |
| **SEC-01.2** | Global daily send ceiling as a circuit breaker. Caps the Resend bill and the reputation damage; converts an unbounded incident into a bounded one. Fail closed and alert rather than silently absorbing. | `[DEV]` ✅ | `worker/src/ratelimit.js`, `worker/src/index.js` | S | SEC-01.1 |
| **SEC-01.3** | Cloudflare Turnstile on both forms. The honeypot stops bots that fill every field; it stops nothing that reads the form once. Turnstile sets no cookies and does no behavioural profiling — which is why it fits here and reCAPTCHA does not. Keep the honeypot alongside it. | `[DEV]` ✅ | `worker/src/turnstile.js` (new), `worker/src/index.js`, `index.html`, `contribute/index.html`, `worker/wrangler.toml`, `scripts/check-origins.mjs` (new allowlist entry) | M | — |
| **SEC-01.4** | Bound `updateWithRetry`'s write amplification — up to five GitHub API round trips per confirmation, against a 5,000/hr ceiling, failing silently. Add backoff between attempts; log exhaustion loudly. | `[DEV]` ✅ | `worker/src/github.js` | S | — |

**Caught during implementation, not in the original spec:** the first draft of SEC-01.3 defined the Turnstile widget's `ref` callback as a fresh arrow function inline in `renderVals()` — same pattern the codebase already uses for `onChange`/`onClick` handlers. React re-invokes a `ref` callback (with `null`, then the new value) whenever its *identity* changes between renders, unlike ordinary event-handler props — so that draft would have torn down and rebuilt the widget on every keystroke elsewhere in the form, discarding a solved token the moment someone typed their name. Fixed by making `turnstileRef` a stable class field (`turnstileRef = (el) => this.mountTurnstile(el)`, assigned once) in both `index.html` and `contribute/index.html`. Left as a comment at both sites so the next person touching either `renderVals()` doesn't reintroduce it by copying the surrounding pattern.

**What's actually live vs. not yet:**
- All four items are code-complete, tested (`worker/test/abuse.test.mjs`, new — 8 tests covering per-IP, per-address, daily cap, Turnstile pass/fail/missing-secret, honeypot bypass, and fail-open with no KV bound), and merged to this branch.
- **Not live in production yet — cannot be, from this session.** Two real Cloudflare account actions gate it, neither of which this session has credentials for: (1) create the `RATE_LIMIT` KV namespace and paste its id into `wrangler.toml`, (2) register a Turnstile widget for the domain and set the real `TURNSTILE_SITE_KEY` (in both HTML files, currently `"CHANGE_ME_after_registering_a_turnstile_widget"`) and `TURNSTILE_SECRET_KEY` (Worker secret). `worker/README.md` Steps 6–9 now cover both, inserted into the existing setup sequence rather than appended.
- **Do not deploy the Worker or merge this branch to `main` until both are done together.** The Worker code fails closed on subscribe when `TURNSTILE_SECRET_KEY` is unset (by design — see SEC-01.3's test "a Worker with no Turnstile secret configured refuses rather than skipping verification"), so deploying the Worker alone breaks live signups entirely rather than just leaving them unprotected. Deploying only the site with the placeholder key breaks it from the other side (Cloudflare's own script will not render against an invalid site key). This is a **coordinated deploy**, not two independent ones.
- `npm run check` (`scripts/check-pages.mjs`) will show a console-error problem on `index.html` and `contribute/index.html` in **any environment with real internet access** until the real `TURNSTILE_SITE_KEY` is set — Cloudflare's script validates the site key client-side and errors on the placeholder string. (In this dev sandbox specifically, the same check currently fails for an unrelated reason too — the sandbox's browser cannot reach any external host at all, proven identical to `example.com` failing the same way — but that sandbox-only limitation is pre-existing, documented in this file's Definition of Done section already, and resolves in real CI regardless of the Turnstile key.) **This is expected, not a regression** — don't chase it without first checking whether the real keys are in place.
...
| ID | Task | Tags | Files | Effort | Depends on |
|---|---|---|---|---|---|
| **SEC-02.1** | Take name and interests out of the confirm token. Store the pending signup in KV under a random opaque ID with a 48h TTL; put only that ID in the link. Restores the aggregate-only property `store.js` already documents as the design goal. | `[DEV]` | `worker/src/index.js`, `worker/src/crypto.js`, `worker/test/flow.test.mjs` | M | — |
| **SEC-02.2** | **Decide the git-history question.** Unsubscribing removes from `HEAD`; the encrypted record persists in history forever. Three defensible answers — periodically squash the store repo's history, migrate the store to KV/D1/R2 where deletion is deletion, or keep it and say so in the fault list. Silence is not one of them. | `[DECISION]` `[DEV]` | `worker/src/store.js`, `hugo/data/`, store repo | M–L | author call |
| **SEC-02.3** | Interstitial confirm button so `GET /api/confirm` stops being state-changing. Mail scanners, SafeLinks and antivirus prefetchers fetch every URL in an email and will confirm subscriptions no human confirmed — silently breaking the double opt-in the site advertises. **Absorbs SUGGEST-04**, which raised this independently. | `[DEV]` | `worker/src/index.js` | S | — |
| **SEC-02.4** | Update the Behind the Scenes fault list and changelog to match whatever 10.2 actually lands, in the site's own `~~was:~~ now:` voice. | `[COPY]` | `hugo/layouts/behindthescenes.html`, `hugo/data/` | S | SEC-02.1–02.3 |
...
| ~~**SUGGEST-04**~~ | ~~Harden the dispatch Worker's `/api/confirm` and `/api/unsubscribe` against link-prefetching (email security scanners, Outlook Safe Links, some VPN/antivirus products fetch links in email bodies automatically). Currently both act on a bare `GET`. An interstitial confirm button (still one click, still no account, still honors "one click to leave") would prevent a scanner from silently confirming or unsubscriming someone.~~ — **absorbed into SEC-02.3 (Phase 10), 2026-08-11.** Not done; re-filed with the rest of the Worker hardening so the dispatch security work sits in one place. | Raised here first, during codebase familiarization, and independently re-found by the 2026-08-11 security review — which is some evidence it's real. Note the security framing is slightly different from the original: the issue isn't only nuisance, it's that a scanner auto-confirming breaks the **double opt-in the site publicly advertises**, so it's a claim-vs-behaviour gap, not just a hardening nicety. |
...
- [ ] Confirm/unsubscribe links resist prefetch-triggered false actions (SEC-02.3, once implemented — was SUGGEST-04)
- [ ] `/api/subscribe` refuses a burst from one address and from one IP (SEC-01.1), and the daily ceiling holds (SEC-01.2)
- [ ] A freshly issued confirm token decodes to an opaque ID and nothing else — no address, no name, no interests (SEC-02.1)
- [ ] The double opt-in claim is literally true: a link fetched by a scanner does not confirm anyone (SEC-02.3)
- [ ] Whatever was decided about git history (SEC-02.2) matches what the unsubscribe email and the fault list say
