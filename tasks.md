...
### Mechanism — build first

| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **HHO-02** | Crypto module: PBKDF2-HMAC-SHA-256 (600 000 it.) → one AES-256-GCM key per file, envelope AAD, GCM-tag-failure error path. No HKDF split, no digest, no padding — see spec §7. | `[DEV]` | new, under `/practise/hot-honest-ours/` | M | |
| **HHO-03** | EFF long wordlist (7 776 words) embedded; 5-word passphrase generator from `crypto.getRandomValues`; light input normalisation (NFKC/lowercase/separator-collapse) on the compare screen's passphrase fields. | `[DEV]` | same | S | |
| **HHO-04** | Consent state model: three states per question (private default / match-only / reveal), the boundary override (a shared NO always reveals), type restrictions (no match-only on free text) — spec §5. | `[DEV]` | same | M | |
| **HHO-05** | `.hho` envelope + payload per spec §6: only shared questions get a key, private/unanswered are simply absent, no `pair` field, no `pad` field. Validation per §6.4, rendered from the local questionnaire, never from the file. | `[DEV]` | same | M | Security-relevant: never build UI from parsed file content. |
| **HHO-06** | Share flow screens: choose what to share, review what will leave (literal content, not a count), protect it (passphrase display, "send file and passphrase separately"), file-ready/hand-off with `navigator.share` where supported — spec §4.1–4.5. | `[DEV]` `[COPY]` | same | M | Copy is drafted verbatim in the spec; near-mechanical to implement. |
| **HHO-07** | Compare flow screens: open my file / open their file, **two independent passphrase fields** (not one shared passphrase — spec §5.8), paste-text fallback for iOS, error states per §4.6's table. | `[DEV]` `[COPY]` | same | M | |
| **HHO-08** | Comparison engine: boundary axis + tier axis (collision/different/matched/only-one-of-us/not-compared), plain-value equality for match-only (no digest), free text excluded from tiering, version-mismatch intersection — spec §8.2–8.3, §8.7. | `[DEV]` | same | M | |
| **HHO-09** | Comparison results UI, adopting the supplied `Compare_Sheet.dc.html` per spec §8.1's keep/change/reject list: triage tiles, grids, ordinal dot-plots with visually-hidden text equivalents, boundary-first ordering, demo mode, the sheet's own closing line verbatim. **Reject the sheet's proportional alignment bar** (reads as a compatibility score) and its `localStorage`-based partner read. | `[DEV]` | same | L | |
| **HHO-10** | CSP for the new route(s): no dc-runtime, no `unsafe-eval`, `connect-src 'none'`, external stylesheet only (no `unsafe-inline`) — spec §9.2–9.3. JS framebusting script as the V1 substitute for `frame-ancestors`. | `[DEV]` | same | S | |
| **HHO-11** | Route wiring: `/practise/hot-honest-ours/`, entries in `scripts/prerender.mjs`, `scripts/sync-base.mjs`, `scripts/check-pages.mjs` (`checkTokens()` file list, plus the no-network and no-cross-origin-subresource assertions spec §14 lists under Testing), `responsive-audit.mjs`. | `[DEV]` | those five files | M | |

### Content — the twelve rounds

| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **HHO-12** | The room: door (what this is / safety five / access check / choose your way in) and all twelve rounds, per UX spec §4–§6. Content is the source worksheet verbatim throughout. | `[DEV]` `[COPY]` | same | L | Largest single item in this phase. Sequenced after the mechanism (HHO-02–11) per HUGO2-03's own "do the stateful, highest-risk part with the mechanism proven first" instinct. |
