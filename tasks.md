...
| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **RS-042** | Practise domain-label correction. The tool holds seventeen general consent domains plus three care/access domains — twenty total — but the current headings ("The seventeen domains," "Access Intimacy & Body Support") don't make that total unambiguous to a reader skimming the page. Rename so the split and the sum are both explicit — e.g. "Seventeen general consent domains" and "Three care and access domains." | `[COPY]` `[DEV]` | `Practise.dc.html` | S | **Implementation order 1** |
| **RS-043** | Resources contents index. A "Choose what you need" list of ordinary same-page anchor links, placed after the immediate-safety questions already on the page, covering: crisis support, coercive control, legal/immigration help, housing/income/food, disability support, 2SLGBTQ+ support, Indigenous-led support, and international resources. Anchors only — no geolocation, no questionnaire, no filtering, no storage. Urgent information stays visible on the page itself, not gated behind the index. | `[DEV]` `[COPY]` | `Resources.dc.html` | S | **Implementation order 2** |
...
**Suggested implementation order, as given:** RS-042 → RS-043 → RS-044 → RS-045 → RS-046 → RS-047/RS-048 if time permits.
