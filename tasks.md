...
| **HHO-26** | **A real gap found while building HHO-15, not introduced by it.** `completed.tasks.md`'s HHO-12 entry states Round 9 ("Three Words") "ships as reference-only text" — it does not; no trace of it exists anywhere in `practise/hot-honest-ours/{index.html,app.js}`. The room currently has 11 reachable rounds, not 12. The spec's own §1.3 terminology table gives the three terms and their fixed meanings (Boundary → what I do, Agreement → what we chose, Rule → an attempt to control), but §6.12 also references "the line under it" and a "Sort yours" invitation from the original worksheet — neither of which is quoted verbatim anywhere in this repo, and the source worksheet itself was pasted into an earlier session, not saved as a file. | `[DECISION]` `[COPY]` | `practise/hot-honest-ours/{questions.js,index.html,app.js}`, `completed.tasks.md` (needs a dated correction note either way) | S | Building from the terminology table alone risks presenting a paraphrase as the source's own words, which this project treats as a real error elsewhere. Needs either the exact original text, or an explicit call that the terminology-table version is good enough. |
