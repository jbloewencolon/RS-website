...
| **HHO-14** | **Shipped in full — see `completed.tasks.md`.** All four of spec §4.1's entry modes are now real choices from the "choose your way in" menu: the long way round, the fridge five, the sixty-second check-in, and the shuffle. | `[DEV]` `[COPY]` | `practise/hot-honest-ours/*` | — | |
| **HHO-27** | **Resolved and shipped, 2026-08-22 — see `completed.tasks.md`.** The 60-second check-in's eight questions and the shuffle's twelve prompts (grouped into the spec's own three named games) are now house-authored, per the author's call — not the original prototype's wording, which was never saved anywhere in this repo. Documented inline in `questions.js`. | `[DEV]` `[COPY]` | `practise/hot-honest-ours/{questions.js,index.html,app.js,style.css}` | — | If the exact source text ever surfaces, replace `CHECKIN` and `SHUFFLE` in `questions.js` verbatim — flagged inline in the code, same pattern as `HHO-26`. |
...
| **HHO-16 (residual)** | **Delisted, not moved — see `completed.tasks.md`.** The map is off `/practise/`'s own nav and tool count (author call, 2026-08-22), but still lives at its unchanged `/practise/#consent-map` anchor, fully functional. **The full spec'd move (§13.1, a real standalone page at `/practise/consent-domains-map/`) is not done and turned out to be far bigger than "one moved file"** — the map and the endings tool share one dc-runtime component and one `state` object in `practise/index.html`; genuinely separating them means extracting shared state, then wiring a tenth route through `site-routes.mjs`, `prerender.mjs`, `check-pages.mjs`, `check-origins.mjs`, and `responsive-audit.mjs`, plus a new redirect stub. | `[DEV]` | `practise/index.html` | M | The one-release-cycle removal this row originally described never starts until the actual move happens — right-scope this as its own task before picking it up, not as "finish HHO-16." |
