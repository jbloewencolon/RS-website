...
| ID | Task | Tags | Files | Effort | Depends on |
|---|---|---|---|---|---|
| **MC-01** | §3.1/§10.1.1 — **Point the responsive checks at the shipped site.** Move the overflow check and the target-size check out of `responsive-audit.mjs` and into `scripts/check-pages.mjs`, which already serves all nine real directory URLs, already launches Chromium correctly and already runs in CI (**MC-C5**). Delete the stub route list rather than extending it (**MC-C2**). Keep the screenshot pass as an on-demand reviewer tool under `check:responsive`, re-pointed at the same nine URLs. Do **not** touch browser discovery (**MC-C1**). | `[DEV]` | `scripts/check-pages.mjs`, `responsive-audit.mjs`, `package.json` | S | — |
| **MC-02** | §8.6/§10.1 exit criteria — **Add the 200% text projects, without which MC-03/04/05 cannot be verified and cannot regress-test** (**MC-C3**). Each of the nine routes at 320×568 and 390×844, at 100% and 200% root font size. Overflow assertion must ignore elements inside an `overflow-x:auto|scroll` ancestor (WCAG 1.4.10 permits Learn's matrix its own two-dimensional scroll) and must ignore off-left skip links. | `[DEV]` | `scripts/check-pages.mjs` | S | MC-01 |
| **MC-03** | §5/§10.1.2 — **Overflow-safe grid primitive.** Add `.cluster-grid` to `head-base.html` using `minmax(min(100%, var(--item-min, 18.75rem)), 1fr)`, then replace the 29 `minmax(<px>` declarations across the nine authored sources, `--item-min` carrying each site's existing minimum so desktop composition is unchanged. Hugo pages via templates + `npm run build:hugo`; the three hand-authored pages via `sync:base`. This is also the first real use of the "deliberately inert on arrival" component layer (§3.2). | `[DEV]` | `head-base.html`, `hugo/layouts/*.html`, `index.html`, `practise/`, `contribute/` | M | MC-02 |
| **MC-04** | **MC-C4(b)** — Unbreakable inline strings overflow at 200% independently of any grid: Manifesto (341px `span`), Practise (`span.sc-interp`, 263/245px — runtime-interpolated, so the fix belongs where the runtime writes, not in static markup). Apply `overflow-wrap:anywhere` / `hyphens:auto` at the text level. Not covered by MC-03. | `[DEV]` | `hugo/layouts/manifesto.html`, `practise/index.html`, `Practise.dc.html` | S | MC-02 |
| **MC-05** | **MC-C4(c)** — Resources' four `h2` rows overflow at 200% as flex children with the default `min-width:auto`. The page already carries a documented `min-width:0` fix at one instance; apply it as a pattern to the flex rows, not a third one-off. | `[DEV]` | `hugo/layouts/resources.html` | S | MC-02 |
| **MC-06** | §3.5/§10.1.3 — **Practise selection controls.** Whole option row becomes the `<label>` target at ≥48px block size with ≥12px separation; native input stays visible and grows to 20–22px (never replaced by colour alone); every editable field to ≥16px computed, which fixes the three 15px text inputs that trigger iOS focus zoom. `scroll-margin-block` on validation targets. **The highest-stakes route on the site** — the audit is right to rank this above navigation work. | `[DEV]` | `practise/index.html`, `Practise.dc.html`, `support.js` | M | MC-02 |
| **MC-07** | §4/Home + §3.5 — Same treatment for Home's 18px newsletter checkboxes and Contribute's form rows. Contribute's "use a different address" button is missing the `min-height:44px` its Home twin has; both migrate to `.action-utility` rather than gaining another inline declaration. | `[DEV]` | `index.html`, `contribute/index.html` | S | MC-03 |
| **MC-08** | §7.4/§10.1.4 — Reserve Turnstile's widget space against CLS, and write the accessible failure/recovery path (content blockers, network failure, JS off). Turnstile is the site's only third-party interaction and its only unreserved layout insertion. | `[DEV]` `[COPY]` | `contribute/index.html`, `index.html` | S | — |
| **MC-09** | §3.6/§10.1.5 — Measure the sticky stack with `ResizeObserver`, publish it as `--sticky-offset`, and drive `scroll-padding-top`/`scroll-margin-top` from it. Retires Archive's fixed `10rem` guesses (`archive.html:89, 133`) and the drift `IA-C2` already documented as untidy-but-safe. Note the audit cites a `STICKY_CLEARANCE` constant in `sections.js` — **no such identifier exists**; the real constants are the CSS values above and `WIDE = 860`. | `[DEV]` | `sections.js`, `hugo/layouts/archive.html`, `head-base.html` | M | MC-02 |
| **MC-10** | §10.1.6 — Physical-device smoke test, iOS Safari + Android Chrome, before this stage is called done. Not simulable: virtual-keyboard reveal, orientation change without data loss in Practise, iOS file picking, Android downloads, print preview. Pairs with the standing `RS-023` residual, which already holds the screen-reader half. | `[DEV]` (needs a person) | all routes | S | MC-01…MC-09 |

**Stage exit:** zero unintended page-level horizontal overflow on all nine routes at 320px **at both 100% and 200% text** (the 200% half is the one that currently fails); no focused control hidden behind sticky UI; Practise operable one-handed; `npm run check` fails on a reintroduced overflow.

### Stage 20.2 — Core mobile UX *(the audit's Phase 2)*

| ID | Task | Tags | Files | Effort | Depends on |
|---|---|---|---|---|---|
| **MC-11** | §3.3 — Compact two-part masthead: wordmark left, 44×44 "Menu" disclosure right, panel opening in normal flow below with the current route marked first. Keep native `<details>`; **do not** hand-add `aria-expanded` (`IA-C3` settled this — the browser manages it). No off-canvas drawer, no bottom bar. | `[DEV]` | `head-base.html`, every page header | M | MC-03 |
| **MC-12** | §3.6/§10.2.2 — Compact local section recovery on Learn, Archive, Behind the Scenes: one 44px row naming the current section, expanding in flow, sticky only after the hero. Replaces the permanently tall contents grid on narrow widths. Deep links, History and no-JS behaviour preserved. Note `IA-01`'s standing constraint: bulk expansion must not open the sexual-content group. | `[DEV]` | `sections.js`, `learn.html`, `archive.html`, `behindthescenes.html` | M | MC-09, MC-11 |
| **MC-13** | §3.4 — Learn's matrix: Full rows becomes the default below the component's own width via container query, with `sections.js` reading the same CSS state instead of its private `WIDE = 860` (§3.4's real point). Chart retained as an explicit "Compare as chart" mode; overflow fade cue; headers sticky inside the scroll wrapper; semantic table markup untouched. | `[DEV]` | `learn.html`, `sections.js` | M | MC-12 |
| **MC-14** | §3.2 — Continue the component migration MC-03 started: shells, stacks, form fields, statuses and disclosures onto the already-declared `.action` / `.action-utility` / `.nav-link` / `.disclosure` / `.form-*` classes. **Incremental and per-component, never a single sweep** — 1,092 inline declarations across nine files, three of which are sync targets rather than Hugo sources. Add the authored/generated/synchronized source map to the README the audit asks for. Container queries for reusable blocks; `@media` reserved for capabilities and the global shell. | `[DEV]` | `head-base.html`, all layouts, `sync-base.mjs`, `README` | L | MC-03 |
| **MC-15** | §4/Archive — Filter chips: start/end fade cue, selected chip scrolled into view, focus preserved on the pressed chip after filtering, one status update, no scroll jump. Empty-state copy plus "clear filters" when a pocket filters to nothing. | `[DEV]` `[COPY]` | `archive.html`, `archive-filter.js` | S | MC-09 |
| **MC-16** | §4/Practise — Wrapping action stack for Print/Save/Reset with destructive reset kept separate and confirmed in-flow (**not** a modal); keyboard-reveal scrolling; orientation continuity; file import/export flows. Web Share only on explicit invocation with download fallback, and copy that states where the file may go without claiming OS guarantees. | `[DEV]` `[COPY]` | `practise/index.html`, `support.js` | M | MC-06 |
| **MC-17** | §3.7 — Safe-area policy: `--shell-left/right` as `max(clamp(...), env(safe-area-inset-*))`; top inset on sticky surfaces only, never on every section. `100dvh` reserved for genuine viewport-bound overlays — there are none today, and this row does not create one. Landscape constraint: no sticky surface taller than 25% of the visual viewport at 568×320. | `[DEV]` | `head-base.html`, `archive.html` | S | MC-09, MC-11 |
