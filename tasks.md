...
| ~~**RS-049**~~ | **Shipped 2026-08-15 — see `completed.tasks.md`.** Behind the Scenes now carries the same pocket-navigation pattern as Learn (IA-20): eight sections closed by default behind a register-coloured hero grid, the sticky `.jump` bar gone. **The five grid-description lines were drafted, not author-supplied** — grounded in the section's own existing wording rather than invented, but not yet confirmed by the author. Flagged in `completed.tasks.md` for review; a one-line swap if any need changing. | `[DEV]` `[COPY]` | `hugo/layouts/behindthescenes.html`, `hugo/data/substrate.yaml` | L | IA-20, sections.js machinery |
