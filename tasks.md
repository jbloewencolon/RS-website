...
| ~~**UX-20**~~ | ~~~40% of the desktop container sits permanently empty on several pages~~ **Reviewed 2026-08-11, kept as-is by author decision.** Re-checked against Learn specifically — the only one of the four pages re-verified, since it's the one that changed most since this audit was written — with real measurements at 1440px, not the original (now stale) data. Confirmed real: the hero and every section's intro prose sit in a ~60ch measure with the right ~45% of the container empty. Also confirmed *not* uniform: the grid-based content (13 principle cards, the four-sense grid, opacity, field guide) already fills the width. No natural secondary content exists to fill the prose gaps without inventing some — unlike Home, which had a real rotating question and dispatch form to place in a right track. Author's call: leave the measure as-is; a 60ch reading width is a deliberate readability choice on the site's densest page, not a defect. | Home, Learn, Practise, Invitation | L | Closed without a code change. Re-open only if a genuine secondary element (a pull quote, a callout) comes along on its own — don't invent one to fill the space. |
