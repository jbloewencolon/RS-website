...
| **RS-040** | New archive group, "Love, eros, and why any of this is worth doing" — Audre Lorde (*Uses of the Erotic: The Erotic as Power*), bell hooks (*All About Love: New Visions*), adrienne maree brown (*Pleasure Activism*), Kai Cheng Thom (*I Hope We Choose Love*), **and Prentis Hemphill** (*What It Takes to Heal* — moved here from RS-033, decided rather than left flagged: RS-033 shipped 2026-08-08 with Menakem alone in "Bodies, regulation, and what happens before words," on the reasoning that five entries in one live group beats two thin ones, per the review's own argument). Closes the erotics gap and adds Black feminist presence the archive currently lacks almost entirely — distinct from RS-006's Spillers/Hartman/Patterson/Roberts genealogy (property and kinship, not love and the erotic; two different holes, per the review). | `[DEV]` `[COPY]` `[VERIFY]` | `hugo/data/archive.yaml` → regenerate `Archive.dc.html` | S–M | All five links (four original plus Hemphill) need independent verification before publishing, same standard as RS-006/007/008/015/033 — Hemphill's was left as `[VERIFY]` in its own source doc too |
...
**New archive entries needing a link (RS-040, `warm-register-review-v2.md` §8)** — Lorde, hooks, brown, Kai Cheng Thom, all four unfilled `[VERIFY]` placeholders in the source review. **Note the overlap**: adrienne maree brown appears in both RS-016's P1 set (*We Will Not Cancel Us*, AK Press) and here (*Pleasure Activism*, AK Press per the review) — same author, two different books, not a duplicate entry. Both are legitimate additions; just don't conflate them when writing the two entries.
