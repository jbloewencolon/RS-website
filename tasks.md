...
| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **HHO-09 (residual)** | Comparison results UI shipped with cards + triage tiles + boundary-first ordering + the closing line, per spec §8.1's keep/change/reject list applied to the supplied `Compare_Sheet.dc.html`. **Not yet built:** the grid layout for dense same-scale groups — now genuinely dense with HHO-12 shipped (13 Want Menu rows, 13 closeness marks, 8 capacity marks, 13 Buffet rows all render as individual cards today rather than a compact grid) — plus ordinal dot-plots with text equivalents and demo mode. All three are explicitly "Should Have — cards work meanwhile" in the spec's own §14 roadmap, so shipping without them isn't a gap against that roadmap, but the grid layout is worth prioritising first now that the content exists to make cards feel long. | `[DEV]` | `practise/hot-honest-ours/{app.js,style.css}` | M | |
