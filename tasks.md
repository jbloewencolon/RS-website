...
| ID | Task | Tags | Files | Effort | Notes |
|---|---|---|---|---|---|
| **RS-045** | Add a contents menu to Behind the Scenes. Ordinary same-page anchor links for: the technical substrate, accessibility/type, crawler policy, reuse terms, current faults, **roadmap** (new section shipped via RS-044, `#roadmap` — not in the author's original list, added because it now exists on the page), change history, and labour/money. Current faults stay visible on the page by default — the menu is navigation, not a way to collapse or hide them. | `[DEV]` | `hugo/layouts/behindthescenes.html` → regenerate `BehindTheScenes.dc.html` | S | **Implementation order 4** |
...
**Suggested implementation order, as given:** ~~RS-042~~ → ~~RS-043~~ → ~~RS-044~~ → RS-045 → RS-046 → RS-047/RS-048 if time permits. RS-042, RS-043, and RS-044 shipped, see `completed.tasks.md`.
