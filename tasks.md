...
| **HHO-16** | Moving/retiring the Consent Domains Map — **still deferred, but its blocking condition just cleared.** The author chose to ship side by side (HHO-15) rather than replace, specifically because HHO-14 wasn't built yet — it now is, in full. Whether to retire the map is a live question again; not decided here, since the original call was to ship side by side, not to replace once HHO-14 landed. | `[DEV]` | new path, `practise/index.html` | S | Not the same as rejected — see `docs/spec/hot-honest-ours-privacy-architecture.md`'s own convention for naming a deferral. |
